...
def func():
    ...
    current_solution = np.array([2,30,1,30], dtype=int)
    ...
    return (f"{amounts[0]:d}  tostadas de pan de semillas o cereales, {amounts[1]:d} gramos de aguacate, {amounts[2]:d} lata(s) de atún + {amounts[3]:d} gramos de frutos secos (almendras/anacardos/cacahuetes/crema de cacahuete...). Las calorías resultantes serían {round(calories):d}. Los macronutrientes resultantes serían {round(macros[0]):d} gramos de carbohidratos, {round(macros[1]):d} gramos de proteínas y {round(macros[2]):d} gramos de grasas.")
