...
def func():
    ...
    food_macros = np.array([[0.0867, 0.8, 0.08],
    [0.82,0.067,0.011],
    [12, 7.8, 0.8],
    [0.054,0.24,0.56]])
    ...
    current_solution = np.array([15,40,1,10], dtype=int)
    ...
    return (f"{amounts[0]:d} gramos de proteína en polvo, {amounts[1]:d} gramos de cornflakes, {amounts[2]:d} vaso (250ml) de leche desnatada y {amounts[3]:d} grs de crema de cacahuete. Las calorías resultantes serían {round(calories):d}. Los macronutrientes resultantes serían {round(macros[0]):d} gramos de carbohidratos, {round(macros[1]):d} gramos de proteínas y {round(macros[2]):d} gramos de grasas.")
