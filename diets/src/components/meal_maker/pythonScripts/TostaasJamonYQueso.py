...
def func():
    ...
    current_solution = np.array([2,4,2,40], dtype=int)
    ...
    return (f"{amounts[0]:d}  tostadas de pan de semillas o cereales, {amounts[1]:d} lonchas de jamón serrano  ({10*amounts[1]:d} gr),  {amounts[2]:d} rodajas de queso Mozzarella ({10*amounts[2]:d} gramos) y {amounts[3]:d} gramos de dátiles. Las calorías resultantes serían {round(calories):d}. Los macronutrientes resultantes serían {round(macros[0]):d} gramos de carbohidratos, {round(macros[1]):d} gramos de proteínas y {round(macros[2]):d} gramos de grasas.")
